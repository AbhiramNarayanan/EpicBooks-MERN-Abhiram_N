...
//since we are using Book inside the route we need to import that to here since we changed the routes from index.js to here so
...
router.post("/", async (request,response)=>{     //to create/save a new book/resource we need to create a new route /books with post http method which we used to create a new resouce lao usec express app so app.post
                    ...
                    const book = await Book.create(newBook);    //then by using book create we created that new book thats why given inside book.create
                    ...
                    return response.status(201).send(book);       // if created return response with a status code also send the book (i mean the details that we given also shown)
                                                                    ...
                                                                    //now if we check database we can see the the collection added to database with the name we added in the url and inside that there is cat at there we can see the data we added through post man ,those objects came to database
...
router.get("/:id",async(request,response)=>{   
                    ...
                    //now going to post man and select get method in the url give url and at end /books/id here we need to use the total books getting books
