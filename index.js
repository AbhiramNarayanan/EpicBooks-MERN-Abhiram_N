...
//for eg: using axios.get(http:localhost...) in the browser at console we can see error like the origin localhost :3000 (i mean the frontend) 
...
app.use(cors()); //so to prevent that restriction we need to use cors package in our server  and use it like this as a middleware
...
//here i am telling like for all routes with /books use middle ware booksRoute.. since we used /books here we can dlt that part path from the routes
//that we placed inside booksRoute bec we only need that at one place dont repeat one thing again so...
